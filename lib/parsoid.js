...
var findApiUrl = function(method, apiVersion, wiki, title, revid) {
	...
	if (apiVersion==='restbase1') {
		// Restbase v1
		console.assert(wiki.restbase || wiki.baseurl, "Bad restbase1 configuration.");
		apiURL = wiki.restbase ||
			url.resolve(wiki.baseurl, '/api/rest_v1/page/html/');
		...
		return { api: 'restbase1', url: apiURL };
		...
		// Parsoid v2
		// Get the host from the baseURL
		var host = wiki.host || url.parse(wiki.baseurl, false, true).hostname;
		console.assert(wiki.parsoid && host, "Bad parsoid2 configuration.");
		apiURL = url.resolve(wiki.parsoid, '/v2/' + host + '/html/');
		apiURL += encodeURIComponent(title); /* encode slashes */
		if (revid) {
			apiURL += '/' + revid;
		}
...
var fetch = function(siteinfo, wiki, title, revid /* optional */, max_redirects /* optional */, status /* optional */) {
	...
	request({ url: apiURL.url, encoding: 'utf8', pool: false, log: this.log }, function(error, response, body) {
...
var parse = function(siteinfo, wiki, wikitext, title, status) {
	...
	return new Promise(function(resolve, reject) {
		...
		request({
			...
			pool: false,
			log: this.log,
			form: (apiURL.api==='parsoid1' ? { wt: wikitext, body: true } : { wikitext: wikitext, bodyOnly: true })
